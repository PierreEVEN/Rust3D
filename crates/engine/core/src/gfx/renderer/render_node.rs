...
use crate::gfx::renderer::renderer_resource::PassResource;
...
pub struct RenderNode {
    ...
    present_pass: bool,
    render_functions: RwLock<Vec<Box<dyn FnMut(&mut Ecs, &CommandCtx, &dyn GfxCommandBuffer)>>>,
}
...
impl RenderNode {
    ...
    pub fn add_render_function<T: FnMut(&mut Ecs, &CommandCtx, &dyn GfxCommandBuffer) + 'static>(&self, render_fn: T) {
        self.render_functions.write().unwrap().push(Box::new(render_fn))
    }
    pub fn draw_content(&self, ecs: &mut Ecs, ctx: &CommandCtx, command_buffer: &dyn GfxCommandBuffer) {
        if let Ok(mut rf) = self.render_functions.write() {
        for render_func in &mut *rf {
            render_func.as_mut()(ecs, ctx, command_buffer);
        }}
    }
}
