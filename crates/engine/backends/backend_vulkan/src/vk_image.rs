                ...
                self.gfx.cast::<GfxVulkan>().device.handle.cmd_copy_buffer_to_image(
                    ...
                    &[vk::BufferImageCopy::builder()
                        ...
                        .image_subresource(vk::ImageSubresourceLayers::builder()
                            .aspect_mask(if self.image_params.pixel_format.is_depth_format() { vk::ImageAspectFlags::DEPTH } else { vk::ImageAspectFlags::COLOR })
                            .mip_level(0)
                            .base_array_layer(0)
                            .layer_count(1)
                            .build())
...
impl VkImageUsage {
    fn from(usage: GfxImageUsageFlags, is_depth: bool) -> Self {
        ...
        if usage.contains(ImageUsage::CopySource) { flags |= vk::ImageUsageFlags::TRANSFER_SRC }
        if usage.contains(ImageUsage::CopyDestination) { flags |= vk::ImageUsageFlags::TRANSFER_DST }
        if usage.contains(ImageUsage::Sampling) { flags |= vk::ImageUsageFlags::SAMPLED }
        if usage.contains(ImageUsage::GpuWriteDestination) {
            flags |= if is_depth { vk::ImageUsageFlags::DEPTH_STENCIL_ATTACHMENT } else { vk::ImageUsageFlags::COLOR_ATTACHMENT }
        }
...
impl GfxImageBuilder<(vk::ImageView, vk::DescriptorImageInfo)> for RbImageView {
    fn build(&self, gfx: &GfxRef, swapchain_ref: &GfxImageID) -> (vk::ImageView, vk::DescriptorImageInfo) {
        ...
        if self.create_infos.read_only {
            let view = vk_check!(unsafe { 
                ...
                .subresource_range(vk::ImageSubresourceRange::builder()
                    .aspect_mask(if self.create_infos.pixel_format.is_depth_format() { vk::ImageAspectFlags::DEPTH } else { vk::ImageAspectFlags::COLOR })
                    .base_mip_level(0)
...
impl VkImage {
    ...
    fn set_image_layout(&self, _: &GfxImageID, command_buffer: vk::CommandBuffer, new_layout: vk::ImageLayout) {
        if self.image_params.read_only {
            ...
            let source_destination_stages = if *current_layout == vk::ImageLayout::UNDEFINED && new_layout == vk::ImageLayout::TRANSFER_DST_OPTIMAL
            {
                barrier.src_access_mask = vk::AccessFlags::NONE;
                barrier.dst_access_mask = vk::AccessFlags::TRANSFER_WRITE;

                (vk::PipelineStageFlags::TOP_OF_PIPE, vk::PipelineStageFlags::TRANSFER)
            } else if *current_layout == vk::ImageLayout::TRANSFER_DST_OPTIMAL && new_layout == vk::ImageLayout::SHADER_READ_ONLY_OPTIMAL
            {
