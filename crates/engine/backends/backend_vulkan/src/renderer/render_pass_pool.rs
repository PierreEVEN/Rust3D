...
impl RenderPassPool {
    pub fn instantiate(&self, render_pass: &RenderPass, initial_res: Vec2u32) -> VkRenderPassInstance {
        ...
        match self.render_passes.read().unwrap().get(render_pass.source()) {
            ...
            Some(found) => {
                VkRenderPassInstance::new(found.clone(), render_pass, initial_res)
            }
        }
    }
    ...
    fn create_if_needed(&self, render_pass: &RenderPass) {
        if !self.render_passes.read().unwrap().contains_key(render_pass.source()) {
            self.render_passes.write().unwrap().insert(render_pass.source().clone(), Arc::new(VkRenderPass::new(render_pass.source())));
