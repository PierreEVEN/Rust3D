...
use crate::gfx::{Gfx, GfxEventBinding};
...
impl<T: Resource + 'static> SwResourceStorage<T> {
    pub fn swapchain() -> Self {
        Self { resources: Arc::new(vec![]), is_per_image: false, reconstruction_factory: RwLock::default() }
    }
}
...
impl<T: Resource + 'static> SwResourceStorage<T> {
        ...
        match factory {
            ...
            Some(factory) => {
                ...
                for sub_resource in *self.resources {
                    let (valid, _resource) = &mut *sub_resource.0.write().unwrap();
                    *valid = false;
                }

                // send command to update instances next frame
                let factory = factory.clone();
                Gfx::get().bind_event(GfxEventBinding::BeforeFrameStart, Frame::new(0), move || {
    ...
    pub fn load<F: ResourceFactory<T>>(&self, factory: F) {
        *self.reconstruction_factory.write().unwrap() = Some((Arc::new(factory)));
...
fn frame_function(frame: Frame) {
    let test = SwResourceStorage::swapchain();

    {
        test.load(TestStrFactory {});
        let data = *test;

        test.is_loading_for(frame.clone());
