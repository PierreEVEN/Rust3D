...
impl ProgramPool {
    pub fn find_or_create_program(&self, pass_id: &PassID, shi: &Arc<dyn ShaderInterface>) -> Option<Arc<dyn ShaderProgram>> {
        let program_uid = ProgramUID::new(pass_id.clone(), shi.get_path());
        match self.existing_programs.get(&program_uid) {
            None => {
                match Gfx::get().create_shader_program(shi.get_path().to_str().unwrap().to_string(), pass_id.clone(), shi.as_ref()) {
